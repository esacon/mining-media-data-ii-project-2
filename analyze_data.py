...
def analyze_language_pair_data(data_dir: str, language_pair: str):
    ...
    if train_file.exists():
        ...
        logger.info(f"Train - Samples: {len(train_df)}, Critical Errors: {sum(train_df['binary_label'])} ({sum(train_df['binary_label'])/len(train_df)*100:.1f}%)")
    else:
    ...
    if dev_file.exists():
        ...
        logger.info(f"Dev - Samples: {len(dev_df)}, Critical Errors: {sum(dev_df['binary_label'])} ({sum(dev_df['binary_label'])/len(dev_df)*100:.1f}%)")
    else:
...
def create_visualizations(results_by_language, output_dir: str):
    ...
    if language_pairs:
        ...
        test_sizes = [results_by_language[lp].get('test', {}).get('total_samples', 0) for lp in language_pairs]
    ...
    if language_pairs:
        ...
        if agreement_data:
            ...
            ax.bar([i - width for i in x], df_agreement['Mean Agreement'], width, 
                   label='Mean Agreement', alpha=0.8)
            ax.bar(x, df_agreement['Full Agreement'], width,
                   label='Full Agreement', alpha=0.8)
            ax.bar([i + width for i in x], df_agreement['Majority Agreement'], width,
                   label='Majority Agreement', alpha=0.8)
...
def main():
    ...
    for lp, results in results_by_language.items():
        ...
        if 'train' in results:
            train = results['train']
            logger.info(f"  Training: {train['total_samples']} samples, "
                       f"{train['critical_errors']} errors ({train['error_rate']*100:.1f}%)")
        
        if 'dev' in results:
            dev = results['dev']
            logger.info(f"  Development: {dev['total_samples']} samples, "
