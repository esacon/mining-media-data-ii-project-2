...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, required=True, help="Path to test data file")
    ...
    parser.add_argument(
        "--output_dir", type=str, default="results", help="Directory to save evaluation results"
    )
        ...
        submission_data.append(
            {
                ...
                "language_pair": (
                    test_data.iloc[i]["language_pair"]
                    if "language_pair" in test_data.columns
                    else ""
                ),
            }
        )
...
def main():
    ...
    setup_logger(
        ...
        log_file=f"{config['paths']['logs_dir']}/evaluate.log",
        log_to_console=True,
    )
    ...
    trainer = Trainer(model=model, tokenizer=tokenizer, config=config, device=device, logger=logger)
    ...
    results = {"predictions": predictions, "probabilities": probabilities, "model_size": model_size}
    ...
    if args.submission_format:
        ...
        generate_submission_file(
            predictions, probabilities, test_dataset.data, submission_path, model_size
        )
